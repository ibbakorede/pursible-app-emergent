...
@dataclass
class SwapRequest:
    """Groups swap request parameters"""
    user_email: str
    from_currency: str
    to_currency: str
    amount: float
...
@dataclass
class SwapContext:
    """Groups swap operation dependencies"""
    wallet_service: Any
...
class SwapService:
    ...
    async def process_full_swap(self, req: SwapRequest, ctx: SwapContext) -> Dict[str, Any]:
        ...
        source_wallet = await ctx.wallet_service.get_wallet(req.user_email, from_curr)
        if not source_wallet:
            return {"success": False, "error": f"No {from_curr} wallet found"}
        if source_wallet.get("available_balance", 0) < req.amount:
        ...
        conversion = self.rate_service.calculate_conversion(req.amount, rate, fee_percent)
        ...
        if not req.confirmed:
            return {
                "success": True,
                "quote": {
                    "fromCurrency": from_curr, "toCurrency": to_curr, "fromAmount": req.amount,
        ...
        # Execute swap
        await ctx.wallet_service.debit_wallet(source_wallet["id"], req.amount)
        dest_wallet = await ctx.wallet_service.get_or_create_wallet(req.user_email, to_curr)
        await ctx.wallet_service.credit_wallet(dest_wallet["id"], conversion["to_amount"])
        
        # Create transaction
        reference_id = self.transaction_service.generate_reference("SW")
        tx_config = TransactionConfig(
            user_email=req.user_email, tx_type="conversion",
            from_currency=from_curr, to_currency=to_curr,
            from_amount=req.amount, to_amount=conversion["to_amount"],
        ...
        # Notify
        notif = NotificationTemplates.swap_completed(
            req.amount, from_curr, conversion["to_amount"], to_curr
        )
        await ctx.notification_service.create_transaction_notification(
            req.user_email, notif["title"], notif["message"], tx["id"]
        )
        ...
        return {
            ...
            "transaction": {
                ...
                "fromAmount": req.amount, "toAmount": conversion["to_amount"],
